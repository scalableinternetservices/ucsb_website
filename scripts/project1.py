...
def page_issues(content, url):
    ...
    # Check for div with id "expert-topics"
    expert_topics_div = soup.find("div", id="expert-topics")
    if not expert_topics_div:
        print("No div with id 'expert-topics' found")
        issues += 1
    else:
        # Check that the div contains an unordered list
        expert_ul = expert_topics_div.find("ul")
        if not expert_ul:
            print("Div with id 'expert-topics' does not contain an unordered list")
            issues += 1
        else:
            # Check that at least one item has a link to /interest/<some_topic>
            li_tags = expert_ul.find_all("li")
            has_interest_link = False
            for li in li_tags:
                link = li.find("a")
                if link and link.get("href"):
                    href = link["href"]
                    if href.startswith("/interest/"):
                        has_interest_link = True
                        break
            
            if not has_interest_link:
                print("No link to /interest/<topic> found in expert-topics unordered list")
                issues += 1
